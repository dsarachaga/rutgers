...
public class BST<Key extends Comparable<Key>, Value> {
	...
	public void deleteAlt(Key key) {
		if (key == null)
			throw new IllegalArgumentException("calls delete() with a null key");
		root = deleteAlt(root, key);
		assert check();
	}

	private Node deleteAlt(Node x, Key key) {
		if (x == null)
			return null;

		int cmp = key.compareTo(x.key);
		if (cmp < 0)
			x.left = deleteAlt(x.left, key);
		else if (cmp > 0)
			x.right = deleteAlt(x.right, key);
		else {
			if (x.right == null)
				return x.left;
			if (x.left == null)
